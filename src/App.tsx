...
import { Home, TrendingUp, Activity, Users, User } from 'lucide-react';
...
type NavTab = 'home' | 'statistics' | 'activities' | 'community' | 'me';
...
function AppContent() {
  ...
  if (!isAuthenticated) {
    return (
      <>
        <AuthPage onAuthSuccess={(userData, accessToken) => login(userData, accessToken)} />
        <TestUserInfo />
      </>
    );
  }
  ...
  const navItems = [
    ...
    { id: 'activities', icon: Activity, label: t.nav.activities },
